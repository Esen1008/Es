...
async def handle_group_message(update, context):
    ...
    username = update.message.from_user.username or f"玩家{str(user_id)[:6]}"
    ...
    if command == '/xz':
        ...
        await update.message.reply_text("请私聊使用送礼功能")
...
async def play_game_group(update, context, bet):
    ...
    save_user_data(user_id, user)
    msg = f"🎲 {username} 下注 {bet:.0f}元\n🎴 你: {show_cards(p_hand)} → {p_name}\n🎴 庄: {show_cards(d_hand)} → {d_name}\n{result_text}\n💰 余额: {user['balance']:.2f}元"
    await update.message.reply_text(msg)
    if is_win and win_amount >= 1000:
        await update.message.reply_text(f"🎊 恭喜 {username} 赢得 {win_amount:.0f}元！")
